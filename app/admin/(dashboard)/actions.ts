...
export async function saveContent(section: ContentSection, formData: FormData) {
  ...
  const imagesRaw = value(formData, "metadata_images");
  let images: string[] | null = null;
  if (imagesRaw) { try { const parsed = JSON.parse(imagesRaw); if (Array.isArray(parsed)) images = parsed; } catch { images = null; } }
  const metadata = { condition: value(formData, "condition"), construction_type: value(formData, "construction_type"), quantity: numberValue(formData, "quantity"), unit: value(formData, "unit"), year: numberValue(formData, "year"), manager_name: value(formData, "manager_name"), phone: value(formData, "phone"), schedule: value(formData, "schedule"), images, status_aktif: value(formData, "status_aktif"), panjang_meter: numberValue(formData, "panjang_meter"), panjang_permanen_meter: numberValue(formData, "panjang_permanen_meter"), panjang_belum_permanen_meter: numberValue(formData, "panjang_belum_permanen_meter"), lebar_meter: numberValue(formData, "lebar_meter") };
